...
def update_latent_viterbi(
    ...
    known_latents = student.list_queries.get(traj_num, {})
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
        ...
        for i, st in enumerate(states):
            ...
            if i in known_idxs:
                ...
                accumulate_logp, max_path[i, :] = (
                    accumulate_logp + torch.zeros([option_dim], device=device),
                    student.annotated_options[traj_num][i] * torch.ones([option_dim], device=device)
                )
            else:
                h = min(known_idxs[known_idxs >i+1], default=None)
                print("Timestep", i, "next index known is", h)
                # import pdb; pdb.set_trace()
                if h is None:
...
def approx_xsat_given_xsaprev_xj(j, t, states, option_model, j_value):
    """P(x_{t+1},s_{t+1},a_{t+1}|x_t, a_t, s_t, x_j)"""
    assert j>t+1, "known timestep j must be larger than t+1"
    ...
    transition_matrix = log_prob_option(states[None, t], option_model)[0] #TODO: t or t+1?
    res = xj_given_xtplus1.T * transition_matrix

    return (res / xj_given_xt)


def xsat_given_xsaprev_xj(j, t, states, actions, option_model, policy, j_value):
    """P(x_{t+1},s_{t+1},a_{t+1}|x_t, a_t, s_t, x_j)"""
    assert j>t+1, "known timestep j must be larger than t+1"
    ...
    transition_matrix = log_prob_option(states[None, t], option_model)[0]
